				 ...
				 unsigned int mediaoff, unsigned int medialen)
{
	...
	if (ct->tuplehash[dir].tuple.src.u3.ip 
		== ct->tuplehash[!dir].tuple.dst.u3.ip)
	{
		struct sip_info * pSip = findSipInfoByMediaPort(ct->tuplehash[dir].tuple.dst.u3.ip, port, &base_port, EXP_DST);
	...
	{
		if (daddr_bk.ip != ct->tuplehash[dir].tuple.src.u3.ip)
		{
			SIP_ALG_DBG("client ip %pI4 != media ip %pI4, sip server forwarding.", &ct->tuplehash[dir].tuple.src.u3.ip, &daddr_bk.ip);
			...
			SIP_ALG_DBG("mask is %pI4", &netmask);
	...
	if (nf_nat_sdp_media && ct->status & IPS_NAT_MASK && !direct_rtp)
	{
		...
		if (NF_ACCEPT == ret && ct->tuplehash[dir].tuple.src.u3.ip != ct->tuplehash[!dir].tuple.dst.u3.ip &&	
				daddr_bk.ip == ct->tuplehash[dir].tuple.src.u3.ip && 
