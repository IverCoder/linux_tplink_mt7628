...
static int url_strhash(const unsigned char *start,
				...
				unsigned int *pIndexBits,
                                unsigned int type);
static int match_fbwifi(const unsigned char *src);
												    ...
												    const struct xt_massurl_info *info)
{
	...
	for (handledSNILen = 0; handledSNILen < SNIListLen; )
	{
		...
		if(HOST_NAME == SNIExt.type && info->type == MASSURL_TYPE_HTTPS && 1 == match_fbwifi(SNIExt.pData))
		{
			return (info->type == MASSURL_TYPE_HTTPS) ? HTTPS_MATCH_SPEED : HTTPS_MATCH_NO_SPEED;
		}
		if (HOST_NAME == SNIExt.type
		 && url_strhash(SNIExt.pData, pSNIExt, (uint32_t *)(info->urlIndexBits), MASSURL_TYPE_HTTP))
		{
			return (info->type == MASSURL_TYPE_HTTP || info->type == MASSURL_TYPE_HTTPS) ? HTTPS_MATCH_SPEED : HTTPS_MATCH_NO_SPEED;
		}
                                                       ...
                                                       const struct xt_massurl_info *info)
{
	...
	if (sslStart >= sslEnd)
	{
		if (MASSURL_TYPE_HTTP == info->type || MASSURL_TYPE_HTTPS == info->type )
		{
...
static int match_fbwifi(const unsigned char *src)
{
	if (src == NULL)
		return 0;
	char *strArr[]={"facebook","fbcdn","akamaihd","analytics","virtualearth",
		"spotilocal","atlassolutions","freeway","static",
		"cdninstagram","doubleclick","adnxs","sitescout","tidaltv",
		"rfihub","fbsbx"};
	int iArr = 0;
	int lenArr = sizeof(strArr)/sizeof(strArr[0]);
	for(iArr; iArr<lenArr; iArr++)
		{
			if (strstr(src,strArr[iArr])!= NULL)
				{
					return 1;
				}
				...
				unsigned int type)
{
	...
	{
	/*
		if((&urlHash[HASH(*pIndex)])->first !=NULL)
		{
			printk(" the url is NOT null and pIndexBits is %d %d %d %d %d %d!!!\n",pIndexBits[0],
				pIndexBits[1], pIndexBits[2],pIndexBits[3],
				pIndexBits[4], pIndexBits[5]);
		}
	*/
...
static bool match(const struct sk_buff *skb, const struct xt_match_param *par)
{
	...
	if (info->type == MASSURL_TYPE_HTTP || info->type == MASSURL_TYPE_URL || info->type == MASSURL_TYPE_HTTPS)
	{
		...
		else if (HTTPS_PORT == ntohs(tcph->dest))
		{
			...
			{	
				 ...
				 */			
				ret = HTTPS_UNMATCH;
				if (MASSURL_TYPE_HTTP == info->type || MASSURL_TYPE_HTTPS == info->type) /*whitelist mode*/
				{
...
static bool checkentry(const struct xt_mtchk_param *par)
{
	...
	if (info->type < MASSURL_TYPE_HTTP || info->type > MASSURL_TYPE_HTTPS )
	{
