...
#include "fast_path.h"
extern uint16_t *hwNatWanVid;
extern uint16_t wanVid;
#endif
...
#define DEFAULT_HW_NAT_WAN_INTF "eth0.2"
static char wan_intf_name[16];
module_param_string(wanIfName, wan_intf_name, sizeof(wan_intf_name), 0400);
MODULE_PARM_DESC(wanIfName, "fast patch wan interface name for HW NAT");
#endif 
...
#ifdef CONFIG_X_TP_VLAN
		return 1;	/* add by wanghao, ignore unknown interface */
#endif
	}
...
uint32_t PpeExtIfPingPongHandler(struct sk_buff * skb)
{
	...
	if (ntohs(veth->h_vlan_TCI) == 0x123 || ntohs(veth->h_vlan_TCI) < 0xB)
	{
		return 1;
	}
#else
...
int PpeHitBindForceToCpuHandler(struct sk_buff *skb, struct FoeEntry *foe_entry)
{
	...
	if (IS_IPV4_HNAT(foe_entry) || IS_IPV4_HNAPT(foe_entry)) {
		...
		/* add by wanghao, we use VID to identify where the pkt go, then we need to get the dev. */
#if defined (CONFIG_RA_HW_NAT_PPTP_L2TP) && defined (CONFIG_RA_HW_NAT_WIFI)
		if (!skb->dev)
		{
			vlanEth = (struct vlan_ethhdr *)LAYER2_HEADER(skb);
			if(ntohs(vlanEth->h_vlan_TCI) > 0xA && ntohs(vlanEth->h_vlan_TCI) < 0x3B)
			{
				skb->dev = DstPort[ntohs(vlanEth->h_vlan_TCI)];
			}
		}
#endif
		/* add end  */
...
/*FP to-lan on/off*/	
...
#endif
	/* add end  */
	else	/* here, we need to make wifi pkt delivered by this branch. */
	{
		//RemoveVlanTag(skb);//do we need to remove the vlan tag?
	}
#endif
		return PpeHitBindForceToCpuHandler(skb, foe_entry);
	...
	/* handle the incoming packet which came back from PPE */
		...
		veth = (struct vlan_ethhdr *)LAYER2_HEADER(skb);
		if((pptp_fast_path || l2tp_fast_path) && (skb->len == (124 - 14)) && ntohs(veth->h_vlan_TCI) == 0x123){//!(ntohs(veth->h_vlan_TCI) > 0xA && ntohs(veth->h_vlan_TCI) < 0x3B)
#else
	...
	/* add end  */
		...
		uint16_t forcePort = 0;
		if (PptpToLanParseLayerInfo(skb, &forcePort)) {/* add forcePort by wanghao  */
		...
		/* add by wanghao */
		if (forcePort > 0xA && forcePort < 0x3B)/* come from WLAN  */
		{
			forcePort = 777;
		}
		else
		...
		if (PpeSetForcePortInfo(skb, foe_entry, forcePort)) {/* use forcePort instead 1, by wanghao  */
			memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
			dev_kfree_skb_any(skb);
			return 0;
		}
...
int32_t PpeFillInL4Info(struct sk_buff * skb, struct FoeEntry * foe_entry)
{
	/* add by wanghao */
#if defined (CONFIG_RA_HW_NAT_PPTP_L2TP) 
	if (foe_entry->bfib1.pkt_type != IPV4_HNAPT)
	{
		return 1;
	}
#endif
...
#endif
/* added by wuguocheng, for MT7612 */
#if defined (CONFIG_APCLI_SUPPORT)
	else if (strncmp(skb->dev->name, "apclii", 6) == 0) {		
		if (RTMP_GET_PACKET_IF(skb) >= MIN_NET_DEVICE_FOR_APCLI) {
			offset =
				(RTMP_GET_PACKET_IF(skb) -
				 MIN_NET_DEVICE_FOR_APCLI + DP_APCLII0);
...
int32_t PpeTxHandler(struct sk_buff *skb, int gmac_no)
{
	...
	int32_t vlanId = 0;/* add by wanghao  */
     ...
     */
		...
		/* add by wanghao  */
#if defined (CONFIG_RA_HW_NAT_WIFI)
			for (vlanId = DP_RA0; vlanId < MAX_WIFI_IF_NUM; vlanId++)
			{
				if (skb->dev == DstPort[vlanId])
				{
					break;
				}
			}
			
			if (vlanId < MAX_WIFI_IF_NUM)
			{
...
#else
		WanInt = ra_dev_get_by_name(wan_intf_name);
#endif
...
#ifdef CONFIG_X_TP_VLAN
	/* check wan_Intf_name */
	if (wan_intf_name[0] == 0)
	{
		strncpy(wan_intf_name, DEFAULT_HW_NAT_WAN_INTF, sizeof(wan_intf_name));
	}

	if (ra_dev_get_by_name(wan_intf_name) == NULL)
	{
		strncpy(wan_intf_name, DEFAULT_HW_NAT_WAN_INTF, sizeof(wan_intf_name));
	}
#endif	
