...
extern int get_tx_ring_usage();
extern int get_tx_ring_size();

void sfe_tx_delay()
{
	int threshold = get_tx_ring_size() * 8 / 10;
	int usage = get_tx_ring_usage();
	if(usage >= threshold) 
	{
		udelay(20);
	}
			     ...
			     unsigned int len, struct sfe_ipv4_ip_hdr *iph, unsigned int ihl, bool flush_on_find)
{
	...
	if ((cm->flags & SFE_IPV4_CONNECTION_MATCH_FLAG_XLATE_SRC) &&
		!(cm->flags & SFE_IPV4_CONNECTION_MATCH_FLAG_XLATE_DEST) ) {
		if(stat_cb) {
			stat_cb(skb);
		}
	}
	...
	if (!ipv4_fwd_cb && cm->connection->mark != 0) {
		if (skb->mark & SFE_MASK_FOR_WAN2LAN) {
			skb->mark = cm->connection->mark | SFE_MASK_FOR_WAN2LAN;
		} else {
...
#if defined(CONFIG_RALINK_MT7628)
	if (cm->flags & SFE_IPV4_CONNECTION_MATCH_FLAG_WRITE_L2_HDR) {
		sfe_tx_delay();
	}
#endif
