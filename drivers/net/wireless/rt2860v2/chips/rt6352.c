...
INT RT6352_set_ed_cca(RTMP_ADAPTER *pAd, BOOLEAN enable)
{
	...
	if (enable)
	{
		...
		bbp_val = 0xb; // bit 0~7 for high threshold
	...
	if ((pAd->CommonCfg.MO_Cfg.bDyncVGAEnable) && (pAd->bCalibrationDone))
	{
		...
		if (pAd->RalinkCounters.OneSecFalseCCACnt > pAd->CommonCfg.MO_Cfg.nFalseCCATh)
		{
			...
			else if (BbpReg == (pAd->CommonCfg.MO_Cfg.Stored_BBP_R66 + VgaGainLowerBound))
			{
				...
				if((pAd->RalinkCounters.OneSecFalseCCACnt > pAd->CommonCfg.MO_Cfg.nFalseCCATh)&&(pAd->chipCap.avg_rssi_all > -50))
		    	{
		    	 	BBP_IO_READ8_BY_REG_ID(pAd, BBP_R65, &BbpR65);
					if (BbpR65 == 0x2C)
					{
						DBGPRINT(RT_DEBUG_OFF,("CurrOneSecFCCACnt %d, CurrARSSI %d,  Set R65 to 0x28 from 0x2C (Middle LNA)\n", 
							                           pAd->RalinkCounters.OneSecFalseCCACnt, pAd->chipCap.avg_rssi_all));
						BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R65, 0x28); /* Middle LNA Gain */
					}
		    	}
			}
		}
		else if (pAd->RalinkCounters.OneSecFalseCCACnt < pAd->CommonCfg.MO_Cfg.nLowFalseCCATh)
		{
			...
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x9D);
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R196, 0x27);
			if(pAd->chipCap.avg_rssi_all < -50)
			{
				 	BBP_IO_READ8_BY_REG_ID(pAd, BBP_R65, &BbpR65);
					if (BbpR65 != 0x2C)
					{
						DBGPRINT(RT_DEBUG_OFF,("CurrOneSecFCCACnt %d, CurrARSSI %d, Set R65 to 0x2C from 0x28 (High LNA)\n", 
							                       pAd->RalinkCounters.OneSecFalseCCACnt, pAd->chipCap.avg_rssi_all));
						BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R65, 0x2C); /* High LNA Gain */
					}
			}
	...
	for (i = 0; i < mcs_pwr_table.NumOfEntries; i++)
	{
		...
		{	
			for (j = 0; j < 8; j += 2)
			{
				...
				INT32 rate_pwr = 0, rate_pwr_ori = 0, sku_rate_pwr, pa_mode = 0, sku_rate_pwr_20in40 = 0;
				INT mcsIndex = 0;
				...
				{
					case 0x1314: 						
						/* for bug that power of 11b is 1.5dbm higher than 11g.
						rate_pwr += ((j == 0) || (j == 2)) ? mcs_digpwr_diff : 0;
						pa_mode = ((j < 4)) ? 
							pAd->rf_pa_mode_over_cck[j] : pAd->rf_pa_mode_over_ofdm[j - 4];*/
						pa_mode = ((j < 4)) ? 
								pAd->rf_pa_mode_over_ofdm[j] : pAd->rf_pa_mode_over_ofdm[j - 4];
					break;
				...
				sku_rate_pwr_20in40 = (INT32)GetSkuPerRatePwr(pAd, mcsIndex, pAd->CommonCfg.Channel, BW_20, pa_mode_table[pa_mode]);
				DBGPRINT(RT_DEBUG_TRACE,
					("%s::sku_rate_pwr = %d, sku_rate_pwr_20in40 = %d, pAd->BW_Power_Delta=%d\n", 
					__FUNCTION__,sku_rate_pwr, sku_rate_pwr_20in40,pAd->BW_Power_Delta));
				DBGPRINT(RT_DEBUG_TRACE,
					("%s::pAd->CommonCfg.SkuChannel = %d, pAd->CommonCfg.BBPCurrentBW = %d\n", 
					__FUNCTION__, pAd->CommonCfg.SkuChannel, pAd->CommonCfg.BBPCurrentBW));
				/* only 11n needs to use CommonCfg.BBPCurrentBW; others use 20M */
				if (mcsIndex < SKU_PHYMODE_HT_MCS0_MCS1)
					{
				sku_rate_pwr = sku_rate_pwr_20in40;
					}
				if((mcsIndex >= SKU_PHYMODE_HT_MCS0_MCS1) && (mcsIndex <= SKU_PHYMODE_HT_MCS14_MCS15))
					{
					pAd->CommonCfg.SkuPower20in40[mcsIndex - SKU_PHYMODE_HT_MCS0_MCS1] = sku_rate_pwr_20in40 - sku_rate_pwr + pAd->BW_Power_Delta;
					}
...
INT32 RT6352_SkuCalculateTxPwrAdj(struct _RTMP_ADAPTER *ad,PTXWI_STRUC pTxwi)
{

	INT32 sku_table_delta = 0;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s ==>current bw %d, real bw %d\n", __FUNCTION__, currentBW, pTxwi->BW));
	DBGPRINT(RT_DEBUG_TRACE, ("%s ==>current PHYMODE %d\n", __FUNCTION__,  pTxwi->PHYMODE));
	

	/* check if currentBW is the same as phy bw, if yes then quit.DO NOT waste time here. */
	if (currentBW == pTxwi->BW )
		return 0;

	MCS = pTxwi->MCS;
			 
	if(currentBW == BW_40 && pTxwi->BW == BW_20 && 
		(pTxwi->PHYMODE == MODE_HTMIX || pTxwi->PHYMODE == MODE_HTGREENFIELD) )
	{
		
		index  = MCS/2;
			
		sku_table_delta = ad->CommonCfg.SkuPower20in40[index];
		DBGPRINT(RT_DEBUG_TRACE, ("%s ==>current sku table delta %d\n", __FUNCTION__, sku_table_delta));
		...
		if (sku_table_delta != 0)
		{
			...
			if (result >=0)
				...
				PwrAdj = 15 + (result + 1)/2;
		}

	}
	DBGPRINT(RT_DEBUG_TRACE, ("%s ==>PwrAdj %d\n", __FUNCTION__, PwrAdj));
	return PwrAdj;
