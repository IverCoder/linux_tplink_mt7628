...
static int qmi_wds_parse(struct cdc_enc_client *client, struct qmi_msg *m)
{
	...
	case 0x0020: /* QMI_WDS_START_NETWORK_INTERFACE_RESP */
		...
		printk("Receive reply of start network interface!\n");
...
int qmi_ctl_request_wds_cid(struct cdc_enc_client *client)
{
	...
	//kernel_debug_callback("%s %d", __FUNCTION__, __LINE__);
	FUNTIONCALL();
	/* return immediately if a CID is already allocated */
	if (state->wds_cid)
		return 0;

	ret = qmi_send_msg(client, QMI_CTL, 0x0022, &tlvreq, 1);
	i = 0;
	while(i < 10000 * 100) 
	{//wait for some time.
		if (state->wds_cid)
...
int qmi_wds_start(struct cdc_enc_client *client)
{
	...
	FUNTIONCALL();
	printk("Call start network interface function!\n");
	if (state->wds_cid == 0) 
		return 0;
	...
	if (strcmp(client->apnName, "default") != 0)
	{
		...
		tlvsize = tlvreq->len + sizeof(struct qmi_tlv);
