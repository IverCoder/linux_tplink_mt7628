...
#if defined(CONFIG_DUAL_UBOOT_SUPPORT)
#define TP_BOOT_SIZE 		(CONFIG_FACTORY_UBOOT_SIZE + CONFIG_SECOND_UBOOT_SIZE)
#define TP_ROOTFS_SIZE		(0x680000 - TP_BOOT_SIZE) 
#else
...
#define TP_ROOTFS_SIZE		0x2e0000
#define TP_KERNEL_SIZE 		0x0f0000
#else	
#define TP_BOOT_SIZE		0x20000
...
static struct mtd_partition rt2880_partitions[] = {
    ...
    {
        name:		"rootfs",
	 size:		TP_ROOTFS_SIZE,/* 0x000000120000-0x0000003e0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE,
    },
    {
        name:		"config",
	 size:		TP_CONFIG_SIZE, /* 0x0000003e0000-0x0000003f0000  */
	 offset:		TP_BOOT_SIZE + TP_KERNEL_SIZE + TP_ROOTFS_SIZE,
    },
    #if 0
    {
...
#elif defined(CONFIG_DUAL_UBOOT_SUPPORT)

static struct mtd_partition rt2880_partitions[] = {
    {
        name:		"boot",
	 size:		CONFIG_SECOND_UBOOT_SIZE,/* 0x000000000000-0x000000020000  */
	 offset:		CONFIG_FACTORY_UBOOT_SIZE,
    },
    {
        name:		"kernel",
	 size:		TP_KERNEL_SIZE,/* 0x000000020000-0x000000160000  */
...
static u_char l_cmp_buf[FLASH_MAX_RW_SIZE] = {0};
...
{
#if defined(CONFIG_MTD_PARTITION_SIZE_4M)
#define FLASH_CONFIG_PARTITION_OFFSET 0x3e0000
#else
#define FLASH_CONFIG_PARTITION_OFFSET 0x7c0000
#endif
	if(spiflash_ioctl_read(l_buf, FLASH_CONFIG_PARTITION_OFFSET, FLASH_MAX_RW_SIZE) < 0)
	{
		return -1;
	}
	...
	if(spiflash_ioctl_write(l_buf, FLASH_CONFIG_PARTITION_OFFSET, 0x10000) < 0)
	{
	...
	return 0;
#undef FLASH_CONFIG_PARTITION_OFFSET
}
...
long flash_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
...
#if defined(CONFIG_DUAL_UBOOT_SUPPORT)
		|| (CONFIG_FACTORY_UBOOT_SIZE == (u32)opt->dest)
#endif
		)
	{
		need_reboot = 1;
	...
	memset(l_buf, 0, FLASH_MAX_RW_SIZE);
    memset(l_cmp_buf, 0, FLASH_MAX_RW_SIZE);
    ...
    case FLASH_IOCTL_WRITE:
	{
        ...
        unsigned int front_len = 0, tail_len = 0;
        ...
        /* opt->dest must be 0x10000 aligned */
		addr = (u32)opt->dest & 0xFFFF0000;					/* first sector addr to erase */
		usr_buf = (unsigned char *)opt->src;
        temp_data = l_cmp_buf;
        ori_data = l_buf;       

        /* opt->dest is 0x10000 aligned, so no front_len */
		for (count = 0; count < sector_num; count++)
		{           
			copy_from_user(temp_data, usr_buf, FLASH_MAX_RW_SIZE);
            spiflash_ioctl_read(ori_data, addr, FLASH_MAX_RW_SIZE);
            
            if (memcmp(ori_data, temp_data, FLASH_MAX_RW_SIZE)) /* content modified */
            {
                spiflash_ioctl_erase(addr, FLASH_MAX_RW_SIZE);
                udelay(20);
                spiflash_ioctl_write(temp_data, addr, FLASH_MAX_RW_SIZE);
                printk("#");
            }
		...
		{
		    spiflash_ioctl_read(ori_data, addr, FLASH_MAX_RW_SIZE);
            memcpy(temp_data, ori_data, FLASH_MAX_RW_SIZE);
			copy_from_user(temp_data, usr_buf, len);
